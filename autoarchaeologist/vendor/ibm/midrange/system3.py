...
class VolumeLabel(ov.Struct):
    '''
       SY21-0045-3, page 1-18 says:

          cylinder sector contents
          […]
          0        08     Volume-label - system directory
          […]

       SY21-0045-3, page 1-19 says:

          This is a 1-sector (256-byte) area located in the third sector
          of cylinder 0 on any 5444. […]

       SY21-0045-3, page 1-12 says:
...
class VtocIndex(ov.Struct):
    '''
       See: SY21-0045-3 page 1-24
...
class VtocIndexSector(ov.Struct):
    '''
       See: SY21-0045-3 page 1-24
    '''
    def __init__(self, tree, lo, **kwargs):
        super().__init__(
            ...
            lo,
            f0_=6,
            f1_=ov.Array(50, VtocIndex, vertical=True),
            f2_=ov.Be16, # S/D address of first member of S list
            f3_=ov.Octet, # Number of format 1's on the S-list
            f4_=ov.Be16, # S/D adress of the last member on the S list
            f5_=ov.Octet, # Number of free tags in the VTOC index
            vertical=True,
        )

class S3X(ov.OctetView):
    '''
       GC21-5162-1, page 4-66 says:

          BLK-block length
             Block length and record length must be equal and one of
             the following values:

             Note: The tape record created is 2 bytes longer than specified
             because a 2-byte logical record number is appended to the tape
             record. […]

       It looks like the logical record number is prepended, big-endian,
       starting with record number 1.

       Possible blocksizes seem to be 3k*(1|2|4) but it is not obvious
       if the two bytes are appended per tape blog or per logical block.
    '''
...
class S3Y(ov.OctetView):
    ...
    def __init__(self, this):
        ...
        if True:
            VtocIndexSector(self, 0x1500).insert()
